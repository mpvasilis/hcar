"""Bayesian QuAcq active constraint-acquisition algorithm built on PyCona's QuAcq."""
import time
...
class BayesianQuAcq(QuAcq):
    def __init__(self, ca_env=None, theta_max=0.9,
        ...
        """Initialize the learner with a BayesianActiveCAEnv, creating one if not provided."""
        if ca_env is None:
    ...
    def learn(self, instance: ProblemInstance, oracle: Oracle = UserOracle(), verbose=0, metrics: Metrics = None):
        """Run Bayesian active learning over the instance, returning the learned ProblemInstance."""
