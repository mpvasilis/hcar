"""Phase 1 passive-learning entrypoint: builds CG, bias, and positive examples for HCAR benchmarks."""
...
def construct_instance(benchmark_name):
    """Construct the (instance, oracle[, overfitted]) tuple for the given benchmark name."""
...
def generate_positive_examples(oracle, variables, count=5):
    """Generate up to `count` distinct positive solutions of the oracle model."""
...
def detect_structured_patterns(variables, positive_examples, grid_size=9):
    """Detect AllDifferent patterns from variable-name structure (grids, multi-dim slices)."""
...
def check_alldiff_in_examples(var_subset, positive_examples):
    """Return True if the variable subset takes all-distinct values in every positive example."""
...
def detect_alldifferent_patterns(variables, positive_examples, use_structured=True,
    ...
    """Detect candidate AllDifferent constraints via structured and/or combinatorial search."""
...
def extract_alldifferent_constraints(oracle):
    """Return the AllDifferent constraints present in the oracle model."""
...
def generate_overfitted_alldifferent(variables, positive_examples, target_alldiffs, count=4, max_attempts=1000):
    """Sample overfitted AllDifferent constraints consistent with the examples but not in the target."""
...
def generate_binary_bias(variables, language):
    """Build the binary bias of all variable-pair relations in the given language."""
...
def prune_bias_with_examples(bias_constraints, positive_examples, variables):
    """Drop bias constraints that are violated by any positive example."""
...
def run_phase1(benchmark_name, output_dir='phase1_output', num_examples=5, num_overfitted=4):
    """Run Phase 1 for a benchmark and pickle the CG, bias, examples, and metadata."""
