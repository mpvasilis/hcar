"""CSPLib model zoo used as the training set for the ML constraint-prior model.

Each ``construct_*`` function returns ``(variables, cpmpy.Model)``. ``CSP_LIBS``
maps a problem id to its constructor. ``train_prior_model.py`` iterates over the
paper's training problems here to build the XGBoost prior model.
"""
import cpmpy as cp
import numpy as np
